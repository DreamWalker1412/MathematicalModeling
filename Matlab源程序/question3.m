...
while count<=iterNum
    ...
    if ~isempty(fesibleB)
        ...
        [route{count,2},route{count,3},route{count,4}] = judgement3(route{count,1},Edge,distMax,theta/delta,V,alpha1,alpha2,beta1,beta2,delta,theta);
        route{count,5}=stackA;
        p = [1,1,0,0,0,0,0];
        stackA = [];
        stackB = [];
        stackC = [];
...
successPossbility =  zeros(iterNum,1);
for i = 1:iterNum
    routeNum(i) = i;
    routeLength(i)=length(route{i,1});
    routeDist(i)= route{i,2};
    successPossbility(i)= route{i,4};
end
routeInfo = [routeNum,routeLength,routeDist,successPossbility];
routeInfo = sortrows(routeInfo,[4 3 2],{ 'descend' 'ascend' 'ascend'});
shortestRouteLength = min(routeLength);  % 最少节点数
shortestRouteDist = min(routeDist); % 最短航迹
maxSuccessPossbility = routeInfo(1,4);
